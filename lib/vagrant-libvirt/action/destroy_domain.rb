      ...
      class DestroyDomain
        ...
        def call(env)
          ...
          if env[:machine].provider_config.disks.empty? &&
            ...
            if have_aliases
              disks_xml.flat_map { |d| REXML::XPath.match(d, 'alias[contains(@name, "ua-box-volume-")]') }.each do |box_disk|
            ...
            env[:machine].provider_config.disks.each_with_index.each do |disk, index|
              ...
              if have_aliases
                domain_disk = disks_xml.lazy.flat_map { |d| REXML::XPath.match(d, 'alias[@name="ua-disk-volume-' + index.to_s + '"]') }.first
                ...
                if !disk[:device].nil?
                  domain_disk = disks_xml.lazy.flat_map { |d| REXML::XPath.match(d, 'target[@dev="' + disk[:device] + '"]') }.first
                  domain_disk = domain_disk.parent if !domain_disk.nil?
                else
